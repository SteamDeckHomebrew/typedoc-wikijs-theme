...
export class WikiJSThemeRenderContext extends MarkdownThemeRenderContext {
  constructor(theme: WikiJSTheme, options: Options) {
    ...
    this.partials["sources"] = (
      reflection: DeclarationReflection | SignatureReflection
    ): string => {
      if (reflection.sources) {
        for (let source of reflection.sources) {
          if (source.url)
            source.url = source.url?.replace(/\/blob\/[^\/]*\//, "/blob/-/");
        }
      }
  ...
  relativeURL = (url: string | undefined): string | null => {
    let ret =
      super
        .getRelativeUrl(url)
        ?.replace(".md", "")
        ?.replaceAll("_internal_", "INTERNAL")
        ?.replaceAll("_", "/")
        ?.replaceAll("INTERNAL", "_internal_") || null;
    if (ret) {
      let retSplit = ret.split("/");
      ret = retSplit[retSplit.length - 1]
    }
    // console.log("got url", url, ret);
    return ret;
  };
}
